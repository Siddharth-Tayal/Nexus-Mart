...
export const allUsers = () => async (dispatch) => {
    try {
        ...
        dispatch({ type: ALL_USERS_FAILS, payload: error.response.data.message });
    }
}
// User Details
export const getUserDetails = (id) => async (dispatch) => {
    try {
        dispatch({ type: UUSER_DETAIL_REQUEST });
        const { data } = await axios.get(`/api/v1/admin/user/${id}`);
        dispatch({ type: UUSER_DETAIL_SUCCESS, payload: data.user });
    } catch (error) {
        dispatch({
            type: UUSER_DETAIL_FAIL,
